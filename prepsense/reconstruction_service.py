"""
Reconstruction Service - PrepSense Ground Truth Reconstruction
...
class ReconstructionService:
    """Service for reconstructing ground truth from telemetry."""
    ...
    def add_noise_to_observed(self, true_prep_time, noise_std=3.0):
        """
        Generate noisy observed signal (baseline system).
        ...
        observed = true_prep_time + epsilon
        return max(0.5, observed)
    
    def reconstruct_from_telemetry(self, telemetry_data):
        """
        Reconstruct KPT from telemetry data.
        
        Parameters:
        -----------
        telemetry_data : dict
            Telemetry data with order_time and pickup_time
        ...
        """
        order_time = telemetry_data['order_time']
        pickup_time = telemetry_data['pickup_time']
        
        kpt_reconstructed = self.reconstruct_kpt(order_time, pickup_time)
        packed_time_hat = self.reconstruct_packed_time(pickup_time)
        
        result = {
            'order_id': telemetry_data.get('order_id', ''),
            'kpt_reconstructed': kpt_reconstructed,
            'packed_time_reconstructed': packed_time_hat,
    ...
    def get_reconstruction_history(self):
        """
